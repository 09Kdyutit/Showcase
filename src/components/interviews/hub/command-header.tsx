    ...
    <div className="rounded-2xl border border-border/60 bg-card p-6 lg:p-8">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="min-w-0">
          ...
          {primaryReadiness ? (
            <div className="flex items-center gap-3 mt-3 flex-wrap">
              ...
              <span className="text-sm text-muted-foreground">{primaryReadiness.sampleLabel}</span>
            </div>
          ) : (
        ...
        <div className="flex flex-col sm:flex-row gap-2 shrink-0">
          {topAction && (
            <Button asChild size="lg" className="gap-2 h-auto py-3 whitespace-normal text-center">
              <Link href={topAction.destination}>{topAction.title} <ArrowRight className="h-4 w-4 shrink-0" /></Link>
            </Button>
          )}
      ...
      {topAction && (
        <p className="text-xs text-muted-foreground mt-4 border-t border-border/60 pt-3">{topAction.reason}</p>
      )}
    </div>
  )
}
