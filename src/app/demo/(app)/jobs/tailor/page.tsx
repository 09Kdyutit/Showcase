  ...
  return (
    <div className={cn('rounded-xl border p-3 space-y-2', riskColor)}>
      ...
      <div className="border-l-2 border-border pl-3">
        <p className="text-xs text-muted-foreground/60 mb-0.5">Source</p>
        <p className="text-xs text-muted-foreground italic">&ldquo;{entry.source_text}&rdquo;</p>
        <p className="text-xs text-muted-foreground/40 mt-0.5">{entry.source_location}</p>
      </div>

      {entry.requires_confirmation && entry.user_confirmed === null && (
        <div className="flex items-center gap-2 pt-1">
          <button onClick={() => onConfirm(true)} className="flex items-center gap-1 text-xs font-medium text-emerald-400 hover:text-emerald-400 transition-colors">
            <Check className="h-3 w-3" /> This is accurate
          </button>
          <span className="text-muted-foreground/30">·</span>
          <button onClick={() => onConfirm(false)} className="flex items-center gap-1 text-xs font-medium text-red-400 hover:text-red-400 transition-colors">
            <X className="h-3 w-3" /> Not accurate
          </button>
        </div>
      )}
      {entry.user_confirmed === true && (
        <p className="text-xs text-emerald-400 flex items-center gap-1"><Check className="h-3 w-3" /> Confirmed accurate</p>
      )}
      {entry.user_confirmed === false && (
        <p className="text-xs text-red-400 flex items-center gap-1"><X className="h-3 w-3" /> Flagged - needs revision</p>
      )}
    </div>
  )
}
...
function DiffBullet({ bullet, onAccept, onReject }: { bullet: TailoredBullet; onAccept: () => void; onReject: () => void }) {
  ...
  return (
    <div className={cn(
      ...
      <div className={cn('leading-relaxed font-medium', bullet.needs_user_input ? 'text-amber-400' : bullet.accepted ? 'text-emerald-400' : 'text-foreground')}>
        {bullet.tailored}
      </div>
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setExpanded(e => !e)} className="text-xs text-muted-foreground/50 hover:text-muted-foreground flex items-center gap-0.5">
        ...
        <div className="flex items-center gap-1.5">
          {bullet.accepted ? (
            <span className="text-xs text-emerald-400 flex items-center gap-0.5"><Check className="h-2.5 w-2.5" /> Accepted</span>
          ) : (
            <>
              <button onClick={onAccept} className="text-xs text-emerald-400 hover:text-emerald-400 flex items-center gap-0.5 transition-colors">
                <Check className="h-2.5 w-2.5" /> Accept
              </button>
              <span className="text-muted-foreground/30">·</span>
              <button onClick={onReject} className="text-xs text-red-400 hover:text-red-400 flex items-center gap-0.5 transition-colors">
      ...
      {expanded && <p className="text-xs text-muted-foreground/60 border-t border-border pt-2">{bullet.reason}</p>}
    </div>
  )
}
      ...
      <div className="border-b border-border bg-surface-50 px-4 lg:px-6 py-3 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {pendingConfirmations > 0 && (
            <Badge variant="outline" className="text-amber-400 border-amber-500/20 text-xs">{pendingConfirmations} to review</Badge>
          )}
          {fabricationRisks > 0 && (
            <Badge variant="outline" className="text-red-400 border-red-500/20 text-xs">{fabricationRisks} to fix</Badge>
          )}
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="hidden md:flex flex-col w-48 border-r border-border bg-surface-50 p-3 gap-1 shrink-0">
          ...
          <div className="mt-auto pt-3 border-t border-border space-y-2">
            <div className="rounded-lg bg-surface-200 p-3 text-xs text-muted-foreground leading-relaxed">
            ...
            {unconfirmed.length > 0 ? (
              <div className="rounded-lg border border-red-500/20 bg-red-500/5 p-3 text-xs text-red-400 leading-relaxed flex items-start gap-1.5">
              ...
              <div className="space-y-6">
                ...
                <section>
                  ...
                  <div className="rounded-xl border border-border bg-surface-100 p-4">
                    ...
                    <button className="flex items-center gap-1 text-xs text-muted-foreground/50 hover:text-muted-foreground mt-2 transition-colors">
                  ...
                  <ol className="space-y-2">
                    {DEMO_TAILORED.interview_brief.behavioral_questions.map((q, i) => (
                      <li key={i} className="flex items-start gap-2 text-sm text-muted-foreground">
                        <span className="text-xs text-muted-foreground/40 shrink-0 mt-0.5 w-4">{i + 1}.</span>{q}
                      </li>
                    ))}
                  </ol>
                </section>
                  ...
                  <div className="space-y-4">
                    {DEMO_TAILORED.interview_brief.star_evidence.map((star, i) => (
                      <div key={i} className="rounded-xl border border-border bg-surface-100 p-4 space-y-2">
                        ...
                        <p className="text-xs text-muted-foreground/50">Source: {star.source_project}</p>
                        {[['Situation', star.situation], ['Task', star.task], ['Action', star.action], ['Result', star.result]].map(([label, content]) => (
                          <div key={label}>
                            <p className="text-xs font-semibold text-brand-400/70 uppercase tracking-wide">{label}</p>
