...
console.log('\n── Spend and reservation guardrails stay in front of the provider call ──')
{
  ...
  const migration = readFileSync(new URL('../supabase/migrations/20260710033040_proofscore_public_capacity.sql', import.meta.url), 'utf8')
  const budgetIndex = route.indexOf('await preparePromptCall(')
  const ipIndex = route.indexOf('await enforceAtomicLimit(')
  const capacityIndex = route.indexOf('await claimDailyCapacity(')
  const providerIndex = route.indexOf('await preparedPrompt.run()')
  assert(ipIndex >= 0 && ipIndex < budgetIndex, 'IP abuse attempts are bounded before they can churn dollar reservations')
  assert(budgetIndex < capacityIndex, 'dollar budget is reserved before public daily/reserved capacity')
  ...
  assert(route.includes('finally {') && route.includes('await preparedPrompt.release()'), 'every pre-provider public exit idempotently releases its dollar reservation')
